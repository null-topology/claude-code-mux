...
| `CCP_AGENT_SUMMARY` | `native` | Claude Code periodically asks for a short progress label for each running background subagent and resends that subagent's whole context with it. The proxy recognizes the prompt, also when trailing system messages follow it. `native` routes and relays the request like any other for its model, with no rewrite, so the client sees its real usage; like any request it can fail (429, 5xx, a spent Codex window). `local` answers it from the transcript, built from the last tool call; the monitor shows provider `local` and the usage reports zero input. `upstream` (`model` and `remote` do the same) pins the provider's junior model at `effort: low`; on the Anthropic route the relayed bytes stay the client's own, so only the model the monitor records changes. Values are trimmed and case-sensitive: exactly `native`, `local`, `upstream`, `model` or `remote`. An empty or unrecognized value is skipped, so the setting falls back to `agentSummary` in `config.json`, then to `native`. `proxy.log` records `agent_summary_forwarded`, `agent_summary_answered_locally` or `agent_summary_routed`. |
