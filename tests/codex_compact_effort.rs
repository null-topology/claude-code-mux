...
use claude_code_mux::providers::codex::translate::request::{TranslateOptions, translate_request};
...
fn normal_request() -> MessagesRequest {
    serde_json::from_value(json!({
        "model": "gpt-5.5",
...
fn translate(request: &MessagesRequest, use_responses_lite: bool) -> Value {
...
fn translate_compact() -> Value {
    translate(&compact_request(), false)
}
...
#[test]
fn global_none_on_a_normal_turn_requests_no_reasoning_artifacts() {
    let _guard = env_lock();
    let config = tempfile::TempDir::new().unwrap();
    let mut env = isolated_environment(config.path());
    env.push(EnvGuard::set("CCP_CODEX_EFFORT", "none"));

    let wire = translate(&normal_request(), false);

    // The proxy-wide override applies to every request, not only compaction.
    assert_eq!(wire["reasoning"], json!({"effort": "none"}));
    assert!(wire.get("include").is_none(), "unexpected include: {wire}");
}

#[test]
fn global_none_on_the_lite_lane_requests_no_reasoning_artifacts() {
    let _guard = env_lock();
    let config = tempfile::TempDir::new().unwrap();
    let mut env = isolated_environment(config.path());
    env.push(EnvGuard::set("CCP_CODEX_EFFORT", "none"));

    let wire = translate(&normal_request(), true);

    // The lite lane always carries a reasoning object for its context mode.
    assert_eq!(
        wire["reasoning"],
...
#[test]
fn named_effort_with_summary_off_still_requests_encrypted_content() {
    let _guard = env_lock();
    let config = tempfile::TempDir::new().unwrap();
    let mut env = isolated_environment(config.path());
    env.push(EnvGuard::set("CCP_CODEX_REASONING_SUMMARY", "off"));
    ...
    let wire = translate(&request, false);

    // Turning the summary off drops only the summary; a real effort still
    // asks for the encrypted content that carries reasoning across turns.
